...
train,test = tj.Sep_TT(test, 0.2)

x_train, y_train, yIR_train = tj.data2x_y_logic(train, Dmedian=50)
x_test, y_test, yIR_test = tj.data2x_y_logic(test, Dmedian=50)
    ...
    for l2 in range(1,5):
        for l3 in range(1,5):
            ...
            model = Sequential([
                ...
                Dense(Hlayer1 , kernel_regularizer=regularizers.l2(0.0001), kernel_initializer='random_uniform'),

                Activation('relu'),
                # Dropout(0.15),
                Dense(Hlayer2,  kernel_regularizer=regularizers.l2(0.0001), kernel_initializer='random_uniform'),

                Activation('relu'),
                # Dropout(0.15),

                Dense(Hlayer3,kernel_regularizer=regularizers.l2(0.0001),  kernel_initializer='random_uniform'),
                Activation('relu'),
                # Dropout(0.15),
            ...
            diff_test = abs(y_test_pre-y_test)


            y_train_pre = model.predict_classes(x_train)
            diff_train = abs(y_train_pre-y_train)
            ...
            plt.subplot(2, 5, (4,5))
            plt.plot(yIR_train,diff_train)

            plt.subplot(2, 5, (4, 5))
            plt.plot(yIR_test, diff_test)
